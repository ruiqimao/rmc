...
var RMC = function () {
  ...
  */
	function RMC(client, config) {
		_classCallCheck(this, RMC);
   ...
   */
		...
		value: function handleMessage(msg) {
			...
			if (message.startsWith(this.config.commandPrefix)) {
				// Get the command.
				var command = message.substring(this.config.commandPrefix.length).split(' ')[0];

				// Get the suffix.
				var suffix = message.substring(command.length + this.config.commandPrefix.length).trim();
    ...
    */
			var generateEntry = function generateEntry(command) {
				...
				if (command in _this2.commands) {
					...
					// Construct the entry.
					var entry = '`' + _this2.config.commandPrefix + command;
